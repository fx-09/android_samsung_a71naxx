...
static irqreturn_t mms_interrupt(int irq, void *ptr);
...
static int secure_touch_init(struct mms_ts_info *info)
{
	...
	register_tui_hal_ts(&info->input_dev->dev, &info->secure_enabled,
				...
				char *read_buf, unsigned int read_len)
{
	...
	buf = kzalloc(write_len, GFP_KERNEL);
	if (!buf)
		return -ENOMEM;

	buff = kzalloc(read_len, GFP_KERNEL);
	if (!buff) {
		kfree(buf);
		return -ENOMEM;
	}
	...
	memcpy(buf, write_buf, write_len);
				...
				unsigned int read_len)
{
	...
	buf = kzalloc(read_len, GFP_KERNEL);
	if (!buf)
		return -ENOMEM;
	...
	while (retry--) {
		res = i2c_master_recv(info->client, buf, read_len);
...
DONE:
	memcpy(&read_buf[start_idx], buf, read_len);
	kfree(buf);
	return 0;
}
...
int mms_i2c_write(struct mms_ts_info *info, char *write_buf, unsigned int write_len)
{
	...
	while (retry--) {
		res = i2c_master_send(info->client, buf, write_len);
...
static void mms_input_close(struct input_dev *dev)
{
	...
	if (!info->enabled) {
		input_err(true, &info->client->dev, "%s tsp is disabled\n", __func__);
		return;
	}
...
static int mms_init_config(struct mms_ts_info *info)
{
	...
	ret = mms_i2c_read(info, wbuf, 2, rbuf, 16);
	if (ret == 1) {
		input_info(true, &info->client->dev, "%s - read product_name fail[%s]\n", __func__, info->product_name);
		return -EIO;
	}
	...
	/* Set resolution using chip info : have to check values on bringup */
...
static int mms_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (ret < 0) {
		input_err(true, &client->dev, "%s [ERROR] mms_init_config\n", __func__);
		goto err_fw_update;
	}
	...
	input_err(true, &client->dev, "%s[%d] called!\n",
		__func__, info->irq);
#endif
...
#ifdef CONFIG_INPUT_SEC_SECURE_TOUCH
	sec_secure_touch_register(info, info->dtdata->ss_touch_num, &info->input_dev->dev.kobj);
#endif
...
{
	input_err(true, &tui_tsp_info->client->dev, "%s, release all finger..\n",	__func__);
