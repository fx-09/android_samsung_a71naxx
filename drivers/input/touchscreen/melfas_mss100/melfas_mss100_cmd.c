...
static void run_trx_short_test(void *device_data)
{
	...
	if (mms_i2c_write(info, wbuf, 3)) {
		input_info(true, &info->client->dev, "%s [ERROR] ASYNC_CMD mms_i2c_write failed\n", __func__);
	} else {
		input_info(true, &info->client->dev, "%s ASYNC_CMD mms_i2c_write success\n", __func__);
	}
...
static void tui_mode_cmd(struct mms_ts_info *info)
{
	...
	input_err(true, &info->client->dev, "%s: %s(%d)\n", __func__, buff,
