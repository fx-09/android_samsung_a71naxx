...
/*
*P-OS TSP FW ver (10 byte)
*IC VENDOR/IC NAME/PROJECT/MODULE VER/FW VER
*ex)ME/30/14/00/01
*/
static void set_format_fw_ver(struct nvt_ts_data *ts, char *buff, int buff_size, u8 fw_ver)
{	
	u8 mpcriteria[32] = { 0 };
	int pid;
	int lcdtype = 0;
	...
	if(lcdtype == 0x126240) module_ver = 0x01; //bring up LCD
	if(lcdtype == 0x008000) module_ver = 0x02; //tianma LCD
			
	snprintf(buff, buff_size, "NO36%02X%02X%02X", pid & 0xFF, module_ver, fw_ver);
}
...
static void get_fw_ver_bin(void *device_data)
{
	...
	set_format_fw_ver(ts, buff, sizeof(buff), ts->fw_ver_bin);
	sec_cmd_set_cmd_result(sec, buff, strnlen(buff, sizeof(buff)));
	sec->cmd_state = SEC_CMD_STATUS_OK;
...
static void get_fw_ver_ic(void *device_data)
{
	...
	data[0] = EVENT_MAP_FWINFO;
	nvt_ts_i2c_read(ts, I2C_FW_Address, data, 2);

	set_format_fw_ver(ts, buff, sizeof(buff), data[1]);
	sec_cmd_set_cmd_result(sec, buff, strnlen(buff, sizeof(buff)));
	sec->cmd_state = SEC_CMD_STATUS_OK;
...
static void factory_cmd_result_all(void *device_data)
{
	...
	snprintf(buff, sizeof(buff), "%d", ts->platdata->item_version);
		...
		struct device_attribute *attr, char *buf)
{
	...
	snprintf(buff, sizeof(buff), "NO%06X", ts->fw_ver_bin);
					...
					char *buf)
{
	...
	ret = nvt_ts_i2c_read(ts, I2C_FW_Address, data, 11);
	if (ret < 0) {
	...
	input_info(true, &ts->client->dev, "%s: sensitivity mode %d,%d,%d,%d,%d\n",
		__func__, diff[0], diff[1], diff[2], diff[3], diff[4]);
...
int nvt_ts_sec_fn_init(struct nvt_ts_data *ts)
{
	...
	if(of_property_read_bool(ts->client->dev.of_node, "novatek,mp-support-dt")) {
		...
		if (ret) {
			input_err(true, &ts->client->dev, "%s: failed to parse mp device tree, pid(TDDI:%04x)\n",
				__func__, pid);
			//return ret;	// bringup panel has no pid in fw, we should use this kind of tsp panel
		}
	}
...
void nvt_ts_print_raw_data(struct nvt_ts_data *ts){
	int *raw_buff;
	int min, max;
	if (mutex_lock_interruptible(&ts->lock)) {
		input_err(true, &ts->client->dev, "%s: another task is running\n",
